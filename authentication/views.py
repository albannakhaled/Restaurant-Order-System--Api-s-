...
def generate_token(request):
    ...
    if user is not None and user.is_superuser:
        ...
        return Response({'message': 'Invalid username or password','status':'false'}, status=status.HTTP_400_BAD_REQUEST)
...
def register(request):
    ...
    if user.is_valid():
        if not User.objects.filter(username = data['username']).exists():
            ...
            user.save()
            return Response({
                'message':'User created successfully , please verify you\'re email .',
                'status':'true',
...
def is_authenticated(request):
    return Response({'message':'User is authenticated','status':'true'},status=status.HTTP_200_OK)
...
def login(request):
    ...
    if user.is_active:
        if user.check_password(data['password']):
            ...
            return Response({'message':'Invalid username or  password','status':'pfalse'},status=status.HTTP_400_BAD_REQUEST)
    else:
        return Response({'message':'Please verify your email first','status':'false'},status=status.HTTP_400_BAD_REQUEST)
...
def get_verification_link(request, user):
    ...
    return f'{domain_name}/auth/verify/{uidb64}/{token}/'
